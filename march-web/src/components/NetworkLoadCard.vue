...
const loadData = async () => {
	const result = await http.getNetworkLoad(
		state.dateStart,
		state.dateEnd,
...
onMounted(() => {
	...
	chart = new Line('chart-network-load', {
		data: [],
		xField: 'datetime',
		yField: 'value',
		seriesField: 'name',
		...
		yAxis: {
			label: {
				formatter: (text: string) => {
					return utils.formatBytes(parseInt(text));
				},
			},
		},
		tooltip: {
			formatter: (datum: Datum) => {
				return {
					name: datum.name,
					value: utils.formatBytes(parseInt(datum.value)),
				};
			},
		},
		animation: false,
